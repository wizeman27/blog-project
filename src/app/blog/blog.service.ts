...
import { Blog, BlogBody, BlogDraft, BlogMeta } from './blog.model';

@Injectable({
  providedIn: 'root',
})
export class BlogService {
  blogsChanged = new Subject<BlogMeta[]>();
  tagsChanged = new Subject<Set<string>>();
  featuredBlogsChanged = new Subject<BlogMeta[]>();
  ...
  private blogsMeta: BlogMeta[] = [
    ...
    new BlogMeta(
      ...
      'Published',
      'en',
      'https://source.unsplash.com/HB-Kf9WLy_0',
    ...
    new BlogMeta(
      ...
      'Published',
      'en',
      'https://source.unsplash.com/9jsV5uKbAEM/',
      ['Tag1', 'Tag2'],
      '2'
    ),
    new BlogMeta(
      ...
      'Draft',
      'en',
      'https://source.unsplash.com/pz_hAv6ER7c/',
      ['Tag1', 'Tag2'],
      '3'
    ),
    new BlogMeta(
  ...
  getBlogsMeta(): BlogMeta[] {
    // only show published posts
    return this.blogsMeta.filter((blog) => blog.status === 'Published');
  }

  getAllBlogsMeta(): BlogMeta[] {
    // return all available posts (published and draft)
    return this.blogsMeta.slice();
  }

  getAllBlogsBody(): BlogBody[] {
    // return all available posts (published and draft)
    return this.blogsBody.slice();
  }
  ...
  getFeaturedBlogsMeta(): BlogMeta[] {
    return this.blogsMeta.filter(
      (blog) => blog.featured && blog.status === 'Published'
    );
  }

  getBlogMeta(address: string): BlogMeta {
    return this.blogsMeta.filter((blog) => blog.address === address)[0];
  }

  getBlogBody(blogId: string): BlogBody {
    return this.blogsBody.filter((blog) => blog.blogId === blogId)[0];
  }

  getBlogDraft(blogId: string): BlogDraft {
    return this.blogDrafts.filter((blog) => blog.blogId === blogId)[0];
  }

  getTags(): Set<string> {
    const arrayOfTags = this.blogsMeta.map((t) => t.blogTags);
  ...
  getBlogsByCategory(category: string) {
    return this.getBlogsMeta().filter(
  ...
  //   this.blogsChanged.next(this.getBlogsMeta());
  //   this.tagsChanged.next(this.getTags());
  //   if (blog.featured) {
  //     this.featuredBlogsChanged.next(this.getFeaturedBlogsMeta());
  //   }
  // }

  newBlogMeta(blogMeta: BlogMeta) {
    this.blogsMeta.push(blogMeta);
  }

  newBlogBody(blogBody: BlogBody) {
    this.blogsBody.push(blogBody);
  }

  newDraft(blogDraft: BlogDraft) {
    this.blogDrafts.push(blogDraft);
  }
...
updateBlogMeta(id: string, blogMeta: BlogMeta) {
  const index = this.blogsMeta.findIndex((i) => i.blogId === id);
  this.blogsMeta[index] = blogMeta;
  this.blogsChanged.next(this.getBlogsMeta());
  this.tagsChanged.next(this.getTags());
  if (blogMeta.featured) {
    this.featuredBlogsChanged.next(this.getFeaturedBlogsMeta());
  }
}

updateBlogBody(id: string, blogBody: BlogBody) {
  const index = this.blogsBody.findIndex((i) => i.blogId === id);
  if(index >= 0) {
    this.blogsBody[index] = blogBody;
  } else {
    this.newBlogBody(blogBody);
  }
}

updateBlogDraft(id: string, blogDraft: BlogDraft) {
  const index = this.blogDrafts.findIndex((i) => i.blogId === id);
  if(index >= 0) {
    this.blogDrafts[index] = blogDraft;
  } else {
  this.newDraft(blogDraft);
  }
  ...
  deleteBlog(address: string) {
    ...
    const index = this.blogsMeta.findIndex((i) => i.address === address);
    //console.log(index);
    //console.log(JSON.stringify(this.blogs[index]));
    const isFeatured = this.blogsMeta[index].featured;
    this.blogsMeta.splice(index, 1);
    this.blogsChanged.next(this.getBlogsMeta());
    this.tagsChanged.next(this.getTags());
    if (isFeatured) {
      this.featuredBlogsChanged.next(this.getFeaturedBlogsMeta());
  ...
  searchBlogs(query: string) {
    let filteredBlogs: BlogMeta[];
    filteredBlogs = this.blogsMeta.filter(
      (blog) =>
